...
When you add a repo to a Code session, the Claude GitHub App is what grants access — per-repo, granted by you, revocable by you. Through it, Code reads the repo, works on branches, and opens PRs. It **cannot merge** — not as a courtesy, but by construction. This is the zero-setup path: add the repo, start typing. One small thing you'll notice on the PRs it opens: a little "Generated by Claude Code" line at the bottom of the description — that's the tool signing its work, it's expected, and it changes nothing about the change itself.
...
GitHub **Actions** are small workflows that run automatically on your repo — your factory's guardrails check every PR this way, and your deploys can ride the same rail. When a workflow needs a credential (a deploy token, a god-view key), the value lives in the repo's encrypted **Actions secrets**, where no human and no seat can read it back out. The workflow uses it; nobody sees it. One switch worth flipping on day one: a workflow that opens pull requests (the factory-update bridge does) needs **Settings → Actions → General → Workflow permissions → Allow GitHub Actions to create and approve pull requests** checked — GitHub ships new repos with it off, and until it's on the run pushes its branch but fails at the PR step.
