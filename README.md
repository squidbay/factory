...
You already pay for a team — you're just using it one chat at a time. This template turns the Claude desktop app into a working factory: **four standing seats** — Cowork, Code, Designer, Coach — that plan, build, design, and audit each other, two on-call specialists, memory that survives every session, and **you as the only gate anything passes**. No code to understand. No command lines. You never leave the chat.
...
- **It notices things while you're away.** A nightly heartbeat takes the factory's pulse — is your factory behind the master, have the tools your seats run on moved, is the memory still healthy — and leaves a **GREEN / FLAG** note. Green means nothing needs you. It only ever reads, it never installs or changes anything, and it's honest about what it can't see rather than guessing. Ask any seat: *"read last night's heartbeat and tell me if anything needs me."*
...
- **One known-good master, and everyone updates from it.** [`squidbay/factory`](https://github.com/squidbay/factory) is the template of record: every improvement lands there first, and every copy pulls from it. Your copy is severed from it on purpose — private, yours, unreachable from here — so improvements arrive by an explicit route rather than on their own. **Three ways, all ending in a pull request you merge:**
  - **Ask.** Say *"check for factory updates"* to your Code seat. It compares your version against the master's and opens one PR with the whole delta.
  - **Wait.** The `factory-update` workflow does the same comparison on the first of each month, so an update finds you in a month you never think to ask.
  - **Get told.** The **nightly heartbeat** checks it every night and says so in its report, so "you're three versions behind" is something you're told rather than something you have to remember to go looking for.

  Want the announcement too? **Watch → Custom → Releases** on the master — Releases, not the whole repo. And nothing here ever auto-updates: your factory works fine on the version you have, and updates are improvements offered, not corrections owed.
