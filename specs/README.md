...
2. **Complete enough that a fresh Code session builds it with zero questions.** Exact files, exact steps, what done looks like, what's out of scope. **Name the source *tree*, not just a repo** — the exact repo *and* the path/subtree the work lives in — so the builder never has to hunt for where a change belongs. If the builder would have to guess, the spec isn't finished — the shape to fill is [`templates/EXECUTE-SPEC.md`](../templates/EXECUTE-SPEC.md).
